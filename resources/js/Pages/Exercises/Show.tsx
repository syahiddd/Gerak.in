...
export default function ExerciseShow({
    ...
    const canEdit =
        ...
        (!exercise.is_system && exercise.created_by === auth.user.id);
    const data = useMemo(
        () =>
            history.map((h) => ({
    ...
    return (
        <AuthenticatedLayout
            header={<h1 className="text-xl font-extrabold">{exercise.name}</h1>}
        >
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                <div className="space-y-4 lg:col-span-2">
                    <Card>
                        <ExerciseMedia exercise={exercise} variant="hero" />
                        ...
                        {!exercise.description && exercise.overview && (
                            <p className="mt-3 text-sm">{exercise.overview}</p>
                        )}
                        ...
                        {exercise.exercise_tips &&
                            exercise.exercise_tips.length > 0 && (
                                <div className="mt-3">
                                    <h3 className="text-sm font-bold">
                                        Coaching cues
                                    </h3>
                                    <ul className="ms-5 mt-1 list-disc text-sm text-zinc-600 dark:text-zinc-300">
                                        {exercise.exercise_tips.map(
                                            (tip, i) => (
                                                <li key={i}>{tip}</li>
                                            ),
                                        )}
                ...
                <div className="space-y-4">
                    ...
                    <Card>
                        ...
                        {prs.length === 0 && (
                            <p className="mt-2 text-sm text-zinc-500">
                                No PRs for this exercise yet.
                            </p>
                        )}
                        {prs.map((pr) => (
                            <p key={pr.id} className="mt-2 text-sm">
                                <strong>{pr.record_type}</strong>:{" "}
                                {String(pr.value_primary)}
